...
if connect.connected():
    channel = send_command(["cat", "/etc/passwd"])
    result = receive_output(channel)

    channel = send_command(["cat", "/etc/passwd", "|", "grep", f"bandit{int(n)+1}"])
    passwd = receive_output(channel)
    passwd = passwd.split(":")
    shell = passwd[6]

    channel = send_command(["cat", shell])
    result = receive_output(channel)

    channel = send_command(["ls"])
    file = receive_output(channel)

    channel = send_command(["cat", file])
    result = receive_output(channel)

    with open(f"./bandit/password/bandit{int(n)+1}-sshkey.private", "w+") as f:
        f.write(result)

    connect.close()

username = username = f"bandit{int(n)+1}"
...
# next part cannot be done in python

result = "c7GvcKlw9mC7aUQaPx7nwFstuAIBw1o1"
