...
def filter_escape(s: str, allowed: list[str] = [" "]) -> str:
    ...
    res = "".join([*filter(lambda t: t.isalnum() or t in allowed, res)])
...
if connect.connected():
    ...
    channel = send_command(["cd", dir, "&&", "git", "tag"])
    tag = receive_output(channel)

    tag = filter_escape(tag)

    channel = send_command(["cd", dir, "&&", "git", "show", tag])
    result = receive_output(channel)

    result = filter_escape(result)
