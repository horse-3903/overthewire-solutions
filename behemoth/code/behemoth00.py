...
if connect.connected():
    ...
    output = re.search('strcmp\((.*?)\)', output)
    output = output.group(1)
    output = output.split(", ")

    output = [re.search('"(.*?)"', o) for o in output]
    output = [o.group(1) for o in output]

    keyword = output[-1]

    channel = send_command([dir])
    
    send_lines([[keyword]], channel)
    output = receive_output(channel, lines=1)

    send_lines([["whoami"]], channel)
    output = receive_output(channel, lines=1)

    send_lines([["cat", f"/etc/behemoth_pass/behemoth{n+1}"]], channel)
    output = receive_output(channel, lines=1)
