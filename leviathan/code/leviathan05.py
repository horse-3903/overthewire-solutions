...
if connect.connected():
    ...
    channel = send_command(["touch", "/tmp/file.log"])
    
    channel = send_command(["echo", "'balls'", ">", "/tmp/file.log"])

    channel = send_command(["ltrace", f"./{file}"])
    output = receive_output(channel, all=True)

    channel = send_command(["ln", "-s", f"/etc/leviathan_pass/leviathan{n+1}", "/tmp/file.log"])
