...
def send_command(command: list[str], connect: pwn.ssh = connect, verbose: bool = True) -> pwn.tubes.ssh.ssh_channel:
...
def receive_output(channel: pwn.tubes.ssh.ssh_channel | pwn.tubes.ssh.ssh_connecter, lines: int = None, all: bool = False, verbose: bool = True) -> str | bytes:
    output = None
...
if connect.connected():
    ...
    channel = send_command(["ltrace", f"./{file}", "0000"])
    output = receive_output(channel, all=True)

    channel = send_command(["ltrace", f"./{file}", "0000"])
    output = receive_output(channel, all=True)

    # run `for i in {0000..9999}; do echo Trying $i; ./leviathan6 $i | grep -v "Wrong"; done;` until program stops

    channel = send_command([f"./{file}", "7123"])

    send_lines([["whoami"]], channel)
    output = receive_output(channel, lines=1)

    send_lines([["cat", f"/etc/leviathan_pass/leviathan{n+1}"]], channel)
    output = receive_output(channel, lines=1)
