...
final_response, final_soup = send_get_request(website=join_website_dir(website, query), username=username, password=password, get_body=False)

content = final_soup.get_text("\n")

content = content.splitlines()

content = [c for c in content if "Password" in c][-1]
